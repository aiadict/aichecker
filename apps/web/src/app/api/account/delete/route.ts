 ...
 *
 * Blocks deletion while an active paid subscription exists rather than
 * cancelling it on the user's behalf: STRIPE_SECRET_KEY is a restricted key
 * with read-only access to Subscriptions (see lib/stripe.ts) by design, so
 * this route genuinely cannot call subscriptions.cancel(). Directing the
 * user through the Customer Portal first (which we do have write access to)
 * also means Stripe's own cancellation confirmation is what the user sees,
 * not something we did silently on their behalf.
 */
export async function POST(req: NextRequest) {
  ...
  const { data: sub } = await admin
    ...
    .eq("user_id", user.id)
    .single<{ status: string; plans: { key: string } | { key: string }[] }>();

  const planKey = sub ? (Array.isArray(sub.plans) ? sub.plans[0]?.key : sub.plans?.key) : "free";
  const hasActiveSubscription =
    planKey !== "free" && sub?.status !== "canceled" && sub?.status !== undefined;
