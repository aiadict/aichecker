...
4. ~~**Domain name**~~ ✅ Done — `werida.io`, contact aliases `hello@werida.io` / `support@werida.io`.
5. ~~**GitHub repo**~~ ✅ Done — [github.com/aiadict/aichecker](https://github.com/aiadict/aichecker).
6. ~~**Deployed**~~ ✅ Done — **https://werida.io** is live on Vercel (project `aichecker`), connected to `main` for auto-deploy on push. Domain DNS was reconfigured without touching the existing Hostinger-hosted email (MX/SPF/DMARC/DKIM for `hello@`/`support@werida.io`) — see `docs/architecture.md`'s Deployment section for exactly what changed. Still only running in **test-mode Stripe** — see item 3.
